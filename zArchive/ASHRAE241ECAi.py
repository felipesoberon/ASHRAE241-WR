...
#    QER_sum = sum(QER(category) for _ in range(n_infected))
    ...
    # Generate and (optionally) print QER values for each infected
    QER_values = [QER(category) for _ in range(n_infected)]
    QER_sum = sum(QER_values)
